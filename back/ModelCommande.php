...
class Commande{
    ...
    public function recupCommande($id_client){
        $prod = $this->bdd->prepare("SELECT link,titre,prix,quantité,id_produit FROM images INNER JOIN produits ON id_produit = produits.id INNER JOIN produitcom ON produits.id = id_produits AND id_mesure = 3 WHERE id_client = :id_client");
