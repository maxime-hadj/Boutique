    ...
    public function verif(){
        ...
        if(isset($_GET["id"])){
            ...
            $verification = $verif->rowCount();
                ...
                if($verification != 0){
                    $prod = $this->bdd->prepare("SELECT link,titre,prix,id_produit FROM images INNER JOIN produits ON id_produit = produits.id INNER JOIN categories ON produits.id_categorie = categories.id AND id_mesure = 3 WHERE produits.id = $id");
                    $prod->execute();
